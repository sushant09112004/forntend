...
function SectionPersonalInfo({ data }) {
  if (!data.personalInfo && !data.name) return null;
  return (
...
function SectionAchievements({ data }) {
...
function SectionCertifications({ data }) {
...
function SectionLanguages({ data }) {
...
const SECTION_RENDERERS = {
  personalInfo: SectionPersonalInfo,
...
export default function ResumePreview({ data, sectionOrder }) {
  ...
  return (
    <>
      ...
      `}} />
      <div
        className="bg-white p-6 md:p-8 shadow-lg rounded-lg max-w-4xl mx-auto"
        id="resume-preview"
      >
        {order.map((sectionId) => {
          const Section = SECTION_RENDERERS[sectionId];
          return Section ? <Section key={sectionId} data={data} /> : null;
        })}
      </div>
    </>
  );
